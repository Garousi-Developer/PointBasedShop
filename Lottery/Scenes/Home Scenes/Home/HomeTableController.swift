...
class HomeTableController: TableController {
    ...
    override func cell(forTableView tableView: TableView, atIndexPath indexPath: IndexPath) -> TableCell {
        ...
        if indexPath.row == 0 {
            ...
            if closestOffers.isEmpty {
                if closestOffersCollectionView.heightHolder == nil {
                    closestOffersCollectionView.heightHolder = closestOffersCollectionView.heightConstraint.constant
                }
                
                topStaticCell.moreButton.alpha = 0
                closestOffersCollectionView.heightConstraint.constant = fonts(.medium).firstLineHeight + scale * 12
                topStaticCell.notFoundLabel.alpha = 1
            }
            else {
                topStaticCell.moreButton.alpha = 1
                closestOffersCollectionView.heightConstraint.constant = closestOffersCollectionView.heightHolder
                topStaticCell.notFoundLabel.alpha = 0
            }
